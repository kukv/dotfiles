---
name: hunk-review
description: Hunk のライブ diff レビューセッションを CLI 経由で操作する。レビュー対象の確認、ファイル/ハンクの移動、セッション内容の再読み込み（reload）、インラインコメントの追加を行う。ユーザーが hunk を起動していて変更を見られるようにと頼んだとき、対話的に diff や変更をレビューしたいとき、hunk セッションを操作したいときに使う。
---
...
これらのルールは同梱スキルの上に重なり、この構成では優先される — 分割ペイン構成（例:
herdr）で、片方があなた（コーディングエージェント）、もう片方が Hunk。多くの場合、複数の
worktree タブを同時に開く。前提は「hunk とエージェントが同じマシンで同じ hunk daemon を
共有していること」で、エージェントや端末マルチプレクサの種類は問わない。正確なコマンド構文は
同梱スキルに委ね、以下のルールはセッションの選択と操作方針だけを縛る。
