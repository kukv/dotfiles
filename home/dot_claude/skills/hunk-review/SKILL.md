...
herdr）で、片方が Claude Code、もう片方が Hunk。多くの場合、複数の worktree タブを同時に
開く。前提は「hunk と Claude Code が同じマシンで同じ hunk daemon を共有していること」で、
端末マルチプレクサの種類は問わない。正確なコマンド構文は同梱スキルに委ね、以下のルールは
セッションの選択と操作方針だけを縛る。
